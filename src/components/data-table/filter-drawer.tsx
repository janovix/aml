			...
			>
				...
				<div className="flex-1 overflow-y-auto p-4 space-y-5">
					{filters
						.filter(
							(filter) =>
								// Skip clientId filter in drawer - it's searchable and handled via ClientFilterPopover
								filter.id !== "clientId" && filter.options.length > 0,
						)
						.map((filter) => {
										...
										{filter.options.map((option) => {
											...
											return (
												<button
													key={option.value}
													onClick={() =>
														onToggleFilter(filter.id, option.value)
													}
													className={cn(
