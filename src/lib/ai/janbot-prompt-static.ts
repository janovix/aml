/**
 * Static tail of Janbot system prompt (LFPIORPI knowledge, boundaries, guidelines).
 * The capabilities / tool list is injected at runtime from tool definitions.
 */
export const JANBOT_PROMPT_STATIC_SUFFIX = `## Tool Usage Strategy
...
4. **Write actions**: Prefer read-only tools. When a tool requires user confirmation (imports and other sensitive actions), wait for explicit approval in the UI before execution completes.
